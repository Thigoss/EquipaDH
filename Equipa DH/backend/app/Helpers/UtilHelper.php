...
class UtilHelper
{
    ...
    public static function formatarDateBrToUs($date)
    {
        $date = explode('/', $date);
        return $date[2] . '-' . $date[1] . '-' .$date[0];
    }

    public static function formatarDateUsToBr($date)
    {
        $date = explode('-', $date);
        return $date[2] . '/' . $date[1] . '/' .$date[0];
    }
    ...
    public static function limparTelefone($valor)
    {
        $valor = trim($valor);
        $valor = str_replace("+", "", $valor);
        $valor = str_replace("(", "", $valor);
     ...
     * @return string
     */
    public static function formatDateBrToUs($date)
    {
        $date = explode('/', $date);
        if (count($date) !== 3) {
            return false;
