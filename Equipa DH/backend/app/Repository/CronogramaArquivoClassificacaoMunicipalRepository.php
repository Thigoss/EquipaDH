...
class CronogramaArquivoClassificacaoMunicipalRepository extends BaseRepository
{

    protected $model;

    public function __construct(CronogramaArquivoClassificacaoMunicipal $model)
    {
        $this->model = $model;
    }
    ...
    {
        if ($data['arquivo']) {
            $documento = $this->salvarArquivo($data['arquivo'], 'documentos/classificacaomunicipal');
