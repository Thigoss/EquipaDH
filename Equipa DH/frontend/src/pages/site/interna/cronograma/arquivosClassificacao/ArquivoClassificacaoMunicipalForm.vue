...
export default {
  name: 'ArquivoClassificacaomunicipalForm',
  components: {},
  ...
  methods: {
    ...
    load (id) {
      this.$http.get(`api/arquivo-classificacao/municipal/${id}`).then(response => {
    ...
    salvar () {
      this.loading = true
      if (this.form.id) {
        this.$http.put('api/arquivo-classificacao/municipal/' + this.form.id, this.form).then(response => {
          if (response.body.success) {
            this.sucesso(response.body.msg)
            setTimeout(() => {
              this.navigate(`/arquivos-classificacao-municipal/${this.$route.params.cronograma}`)
            }, 3000)
          } else {
            this.resetFields()
        ...
        this.$http.post('api/arquivo-classificacao/municipal', this.form).then(response => {
          if (response.body.success) {
            this.sucesso(response.body.msg)
            setTimeout(() => {
              this.navigate(`/arquivos-classificacao-municipal/${this.$route.params.cronograma}`)
    ...
    voltar () {
      if (!this.visualizar) {
        this.$bvModal.msgBoxConfirm('Deseja retornar para tela de pesquisa? É possível que dados informados não sejam salvos.', {
          ...
          if (valor) {
            this.$router.push(`/arquivos-classificacao-municipal/${this.$route.params.cronograma}`)
          }
        })
      } else {
        this.$router.push(`/arquivos-classificacao-municipal/${this.$route.params.cronograma}`)
