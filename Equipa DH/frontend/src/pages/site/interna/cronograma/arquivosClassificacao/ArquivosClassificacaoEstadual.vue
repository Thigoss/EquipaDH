                ...
                <tr class="action" v-for="arquivo in arquivos.data" :key="arquivo.id">
                  ...
                  <td style="width: 1%; white-space: nowrap;">
                    <button class="btn btn-sm btn-primary action" title="Download"
                      @click="$abrirDocumento('arquivo_classificacao_estadual', arquivo.id)">
            ...
            <b-row>
              <b-col cols="4">
                <pagination-page tabindex="0" v-show="arquivos.data !== undefined" :totalPorPagina="totalPorPagina"
                  :resource="resource" :filtros="filtros" :origin="'ArquivosClassificacaoEstadual'"></pagination-page>
              </b-col>
            </b-row>
          </span>
        </b-container>
...
export default {
  name: 'ArquivosClassificacaoEstadual',
  components: {
    Pagination,
    PaginationPage,
    Datepicker
  ...
  created () {
    ...
    let filtros = this.$store.getters.getPageFilter('ArquivosClassificacaoEstadual')
  ...
  data () {
    return {
      totalPorPagina: 20,
      resource: `arquivos-classificacao/estadual/${this.$route.params.cronograma}`,
      cronograma: null,
      filtros: {
        filter: ''
      },
  ...
  methods: {
    ...
    setarFiltros () {
      this.$store.dispatch('updateFilter', { page: 'ArquivosClassificacaoEstadual', filter: this.filtros })
    },
    pesquisar () {
      this.setarFiltros()
    ...
    cadastrar () {
      this.$router.push(`/arquivo-classificacao-estadual/${this.$route.params.cronograma}/cadastrar`)
    },
    editar (id) {
      this.$router.push(`/arquivo-classificacao-estadual/${this.$route.params.cronograma}/editar/${id}`)
    },
    visualizar (id) {
      this.$router.push(`/arquivo-classificacao-estadual/${this.$route.params.cronograma}/visualizar/${id}`)
    },
    alterarSituacao (arquivo) {
      this.$bvModal.msgBoxConfirm(`Tem certeza que deseja ${(arquivo.ativo ? 'inativar' : 'ativar')} o arquivo ${arquivo.nome}? ${(arquivo.ativo ? '' : 'Apenas um arquivo pode estar ativo!')}`, {
        ...
        if (valor) {
          this.$http.put(`api/arquivo-classificacao-status/estadual/${arquivo.id}`).then(response => {
            this.pesquisar()
          }).catch(error => {
            console.log(error)
          })
